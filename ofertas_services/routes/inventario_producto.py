...
from crud.inventario_producto import (
    ...
    get_inventario_productos,
    update_inventario_producto,
    delete_inventario_producto
)
from schemas import InventarioProductoCreate, InventarioProductoOut
...
@router.get("/", response_model=List[InventarioProductoOut])
def leer_inventario_productos(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return get_inventario_productos(db, skip, limit)
